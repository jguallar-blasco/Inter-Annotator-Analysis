...
def calculate_IAA(prep_dict, agreement_dict):
    ...
    for hit in prep_dict:
        ...
        for answer in cur_answers:
            ...
            for compare_answer in cur_answers:
                if answer['argument'][0] == compare_answer['argument'][0]:
                    ...
                    if (answer['startToken'] == -1 and answer['endToken'] == -1):
                        answer_dict['DOES NOT EXIST'] += 1
                    else:
                        answer_dict['EXISTS'] += 1
                    
                    if (compare_answer['startToken'] == -1 and compare_answer['endToken'] == -1):
                        compare_answer_dict['DOES NOT EXIST'] += 1
                    else:
                        compare_answer_dict['EXISTS'] += 1
            ...
            answer_count = []
            answer_count.append(answer_dict['EXISTS'])
            answer_count.append(answer_dict['DOES NOT EXIST'])

            compare_answer_count = []
            compare_answer_count.append(compare_answer_dict['EXISTS'])
            compare_answer_count.append(compare_answer_dict['DOES NOT EXIST'])
        ...
        agreement_dict[hit].append(statistics.median(agreements))
